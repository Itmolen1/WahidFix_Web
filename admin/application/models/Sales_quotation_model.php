...
{
    function sales_quotation_listing_count($searchText = '')
    {
        $this->db->select('BaseTbl.sales_quotation_id,BaseTbl.sales_quotation_created_at,BaseTbl.sales_quotation_status,BaseTbl.sales_quotation_pdf,sr.service_request_ref');
        $this->db->from('tbl_sales_quotation as BaseTbl');
    ...
    function sales_quotation_listing($searchText = '', $page, $segment)
    {
        $this->db->select('BaseTbl.sales_quotation_id,BaseTbl.sales_quotation_status,BaseTbl.sales_quotation_created_at,BaseTbl.sales_quotation_pdf,sr.service_request_ref');
        $this->db->from('tbl_sales_quotation as BaseTbl');
        $this->db->join('tbl_service_request as sr', 'sr.sr_id = BaseTbl.sales_quotation_sr_id','left');
        if(!empty($searchText)) {
            $likeCriteria = "(BaseTbl.vehicle_no  LIKE '%".$searchText."%'
        ...
        $this->db->where('BaseTbl.isDeleted', 0);
        //$this->db->where('BaseTbl.roleId !=', 1);
        $this->db->order_by('BaseTbl.sales_quotation_id', 'DESC');
    ...
    function add_new_purchase_order($val,$sid)
    {
        ...
        'purchase_order_paid_amt'=>0.0,
        'purchase_order_due_amt'=>$val['purchase_order_grand_total'],
    ...
    function purchase_order_add_payment_record($val)
    {
        $data=array(
        'po_payment_record_po_id'=>$val['po_payment_record_po_id'],
        'po_payment_record_date'=>$val['po_payment_record_date'],
        'po_payment_record_invoice_no'=>$val['purchase_order_bill_no'],
        ...
        $this->db->set('purchase_order_due_amt',($val['purchase_order_due_amt']-$val['po_payment_record_due_amt']));
        $this->db->set('purchase_order_paid_amt',($val['po_payment_record_paid_amt']+$val['po_payment_record_due_amt']));
        $this->db->where('purchase_order_id',$val['po_payment_record_po_id']); 
    ...
    function purchase_order_get_payment_record_details($poid)
    {
        $this->db->select('BaseTbl.*,vendor.*');
        $this->db->from('tbl_purchase_order as BaseTbl');
        $this->db->join('tbl_vendor as vendor', 'vendor.vendor_id = BaseTbl.purchase_order_venodr_id','left');
